...
resource "aws_instance" "LearnEd_instance" {
  ...
  user_data = <<-EOF
              ...
              # Clone your LearnEd project from GitHub - an education website
              cd /tmp
              git clone https://github.com/Mukesh-Swami-0612/LearnEd-an-educational-website.git
              
              # Copy website files to Nginx web root
              sudo cp -r LearnEd-website/* /usr/share/nginx/html/
...
resource "aws_instance" "nagios_instance" {
  ...
  user_data = <<-EOF
              ...
              # Create Nagios host definition for LearnEd website - an education website
